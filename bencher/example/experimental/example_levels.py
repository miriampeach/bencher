...
def run_levels_1D(bench: bch.Bench) -> bch.Bench:
    ...
    row = pn.Row()
    row.append(res1.to(bch.TableResult))
    # row.append(res1.to_curve().opts(shared_axes=False))
    row.append(res1.to(bch.CurveResult))
    ...
    for it, r in enumerate(results):
        ...
        ds = r.to_hv_dataset()
        # crv = r.to_curve_ds(ds.data, r.bench_cfg.result_vars[0]).opts(shared_axes=False, height=300)

        # crv *= (
        #     r.to_hv_dataset(bch.ReduceType.NONE)
        #     .to(hv.Scatter)
